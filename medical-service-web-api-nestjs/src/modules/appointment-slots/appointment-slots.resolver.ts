...
export class AppointmentSlotsResolver {
  ...
  @Query(() => AppointmentSlot)
  async appointmentSlot(@Args('id', { type: () => String }) id: string): Promise<AppointmentSlot> {
    return this.appointmentSlotsService.findOne(id);
  }

  @Query(() => [AppointmentSlot])
  async getAppointmentSlotByScheduleId(@Args('id', { type: () => String }) id: string): Promise<AppointmentSlot[]> {
  ...
  @Mutation(() => AppointmentSlot)
  async removeAppointmentSlot(@Args('id', { type: () => String }) id: string): Promise<AppointmentSlot> {
